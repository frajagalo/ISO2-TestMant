...
public class TarjetaCredito extends AbstractTarjeta {
	...
	public void liquidar() {
		double gastos = 0.0;
		final List<MovimientoTarjetaCredito> movemenmt = Manager.getMovimientoTarjetaCreditoDAO().findByTarjetaId(this.idTarjeta);
		for (final MovimientoTarjetaCredito m : movemenmt) {
			if (!m.isLiquidado()) {
